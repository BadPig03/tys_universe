local Lib = TYU
local Smite = Lib:NewModEnchantment("Smite", "SMITE")
...
function Smite:TakeDamage(entity, amount, flags, source, countdown)
    ...
    local count = Lib.Players.GetNullEffectCounts(Lib.ModEnchantmentIDs.SMITE)
    if count == 0 or not smiteList[entity.Type] or (type(smiteList[entity.Type]) == "table" and not smiteList[entity.Type][entity.Variant]) then
        return
    end
