...
local function SetGlobalLibData(value, ...)
    TYU:SetGlobalLibData(value, "Rewind", ...)
end

local function GetGlobalLibData(...)
    return TYU:GetGlobalLibData("Rewind", ...)
end

do
    PrivateField.RoomTypeName = {
...
do
    function PrivateField.TeleportToRoom(id, player, rng)
        if id == RoomType.ROOM_SHOP then
            local roomID = 0
            if Players.AnyoneIsPlayerType(PlayerType.PLAYER_KEEPER_B) then
        ...
        elseif id == RoomType.ROOM_BOSS then
            ...
            if stage <= LevelStage.STAGE4_2 then
                roomList = PrivateField.BossRoomVariant[math.ceil(stage / 2)][stageType]
            ...
            elseif stage >= LevelStage.STAGE5 and stage <= LevelStage.STAGE7 then
                roomList = PrivateField.BossRoomVariant[stage][stageType]
                index = roomList[rng:RandomInt(#roomList) + 1]
        ...
        elseif id == RoomType.ROOM_CHALLENGE then
            if GetGlobalLibData("ChallengeTriggered") then
                return -1
            else
                SetGlobalLibData(true, "ChallengeTriggered")
        ...
        elseif id == RoomType.ROOM_ISAACS then
            ...
            Isaac.ExecuteCommand("goto s."..PrivateField.RoomTypeName[id].."."..roomConfigRoom.Variant)
        end
        TYU.GAME:StartRoomTransition(GridRooms.ROOM_DEBUG_IDX, Direction.NO_DIRECTION, RoomTransitionAnim.TELEPORT, player, 0)
        return 0
    end
end
...
function Rewind:PostNewRoom()
    ...
    if not Utils.IsRoomIndex(GridRooms.ROOM_DEBUG_IDX) and Utils.IsRoomFirstVisit() and PrivateField.RoomTypeName[roomType] then
        local count = GetGlobalLibData("Rooms", tostring(roomType)) or 0
        SetGlobalLibData(count + 1, "Rooms", tostring(roomType))
    end
    if not Players.AnyoneHasCollectible(ModItemIDs.REWIND) or not Utils.IsRoomIndex(GridRooms.ROOM_DEBUG_IDX) then
		return
	end
    for i = 0, 7 do
...
function Rewind:PostNewLevel()
	SetGlobalLibData(false, "ChallengeTriggered")
end
Rewind:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, Rewind.PostNewLevel)

function Rewind:PostEffectInit(effect)
	if not Utils.IsRoomIndex(GridRooms.ROOM_DEBUG_IDX) or not Utils.IsRoomType(RoomType.ROOM_SHOP) then
        return
	end
    effect:Remove()
end
...
function Rewind:PostGridEntityDoorUpdate(door)
	local room = TYU.GAME:GetRoom()
	if not Utils.IsRoomIndex(GridRooms.ROOM_DEBUG_IDX) or not Utils.IsRoomClear() or door:IsOpen() then
        return
	end
    if not Utils.IsRoomType(RoomType.ROOM_SECRET) and not Utils.IsRoomType(RoomType.ROOM_SUPERSECRET) and not Utils.IsRoomType(RoomType.ROOM_ULTRASECRET) then
        return
    end
...
function Rewind:UseItem(itemID, rng, player, useFlags, activeSlot, varData)
	if Utils.HasFlags(useFlags, UseFlag.USE_CARBATTERY) or TYU.LEVEL:GetCurrentRoomIndex() <= GridRooms.ROOM_DEVIL_IDX or TYU.LEVEL:GetAbsoluteStage() == LevelStage.STAGE8 or TYU.LEVEL:GetDimension() > Dimension.NORMAL or TYU.GAME:IsGreedMode() then
	...
	elseif player:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_BELIAL_PASSIVE) and rng:RandomInt(100) < 10 then
		PrivateField.TeleportToRoom(RoomType.ROOM_DEVIL, player, rng)
		return { Discharge = true, Remove = false, ShowAnim = true }
	end
    local rooms = GetGlobalLibData("Rooms")
    if rooms == nil then
        TYU.GAME:StartRoomTransition(GridRooms.ROOM_ERROR_IDX, Direction.NO_DIRECTION, RoomTransitionAnim.TELEPORT, player, 0)
        return { Discharge = true, Remove = false, ShowAnim = true }
    end
