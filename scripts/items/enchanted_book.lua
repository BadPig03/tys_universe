local Lib = TYU
local EnchantedBook = Lib:NewModItem("Enchanted Book", "ENCHANTEDBOOK")
...
local function GrantANewEnchantment(player, rng)
    ...
    if enchantmentID > 0 then
        ...
        if player:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_VIRTUES) or player:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_BELIAL_PASSIVE) then
            Lib.Entities.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_TAROTCARD, Lib.ModCardIDs.ENCHANTEDCARD, Lib.GAME:GetRoom():FindFreePickupSpawnPosition(player.Position, 0, true), nil, nil, rng:Next())
        end
    end
end

function EnchantedBook:PostPlayerHUDRenderActiveItem(player, slot, offset, alpha, scale)
    if not player:HasCollectible(Lib.ModItemIDs.ENCHANTEDBOOK) then
        return
    end
    ...
    if player:GetActiveItem(ActiveSlot.SLOT_PRIMARY) == Lib.ModItemIDs.ENCHANTEDBOOK then
        glintSprite.Scale = Vector(1, 1)
        glintSprite:Render(Vector(20 + 20 * hudOffset, 16 + 12 * hudOffset) + extraOffset)
    end
    if player:GetActiveItem(ActiveSlot.SLOT_SECONDARY) == Lib.ModItemIDs.ENCHANTEDBOOK then
...
function EnchantedBook:PostPickupRender(pickup, offset)
    ...
    if pickup.SubType == Lib.ModItemIDs.ENCHANTEDBOOK and not Lib.Collectibles.IsBlind(pickup) then
    ...
    if pickup.SubType == Lib.ModItemIDs.ENCHANTEDBOOK and Lib.Collectibles.IsBlind(pickup) then
...
EnchantedBook:AddCallback(ModCallbacks.MC_USE_ITEM, EnchantedBook.UseItem, Lib.ModItemIDs.ENCHANTEDBOOK)
