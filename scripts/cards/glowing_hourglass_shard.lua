local Lib = TYU
local GlowingHourglassShard = Lib:NewModCard("Glowing Hourglass Shard", "GLOWINGHOURGLASSSHARD")
...
local function GetGlowingHourglassShards(player)
    ...
    for i = ActiveSlot.SLOT_PRIMARY, ActiveSlot.SLOT_POCKET2 do
        ...
        if card == Lib.ModCardIDs.GLOWINGHOURGLASSSHARD then
...
GlowingHourglassShard:AddCallback(ModCallbacks.MC_USE_CARD, GlowingHourglassShard.PostUseCard, Lib.ModCardIDs.GLOWINGHOURGLASSSHARD)
...
function GlowingHourglassShard:PostEntityTakeDMG(entity, damage, damageFlags, source, countdown)
    ...
    player:UseCard(Lib.ModCardIDs.GLOWINGHOURGLASSSHARD, UseFlag.USE_OWNED)
